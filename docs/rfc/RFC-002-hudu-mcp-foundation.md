...
> Those default-configuration figures are the 0.1.0 measurement and are left as
> recorded. From 0.2.0 the default is 67 registered and 22 withheld, because
> `HUDU_ALLOW_PASSWORD_WRITE` withholds three more tools; the class counts and
> the read-only figure are unchanged. See ADR-003 and `CHANGELOG.md`.
...
| C4  | **Wrong, and corrected after this RFC was written.** As recorded here: a rack's contents cannot be listed, because `RackStorageItem` carries no reference to its rack and `rack_storage_id` does not appear anywhere in the contract | The premises are true of the _item_ and the conclusion does not follow: the rack record itself returns `front_items`/`rear_items`, a per-unit elevation, which no definition in the contract declares (C4 as rewritten, F8). The design consequence recorded at the time — no invented join, no tool claiming a relationship the contract does not carry — was right; the accompanying claim that the question was unanswerable was not, and it reached a tool description as an instruction to refuse |
...
- **Four environment gates**, and gates are environment-only: `HUDU_READ_ONLY`,
  ...
  accepted as a tool argument. **Four was one short, and the gap was corrected
  after this RFC was written.** Nothing gated the write direction to the
  credential vault, so `hudu_create_password`, `hudu_update_password` and
  `hudu_archive_password` registered by default and a deployment could overwrite
  or archive a credential its key could not read. `HUDU_ALLOW_PASSWORD_WRITE`,
  added in 0.2.0, is the fifth gate; the environment-only rule stated here is
  what it was built to satisfy. See ADR-003.
...
- **Central secret stripping.** `stripSecrets` runs in `executeTool` on every
  ...
  null password stays null rather than being disguised as a redaction. **The
  intent was right and the realisation was wrong, and it was corrected after this
  RFC was written.** The placeholder was a sentence written _into_ the field it
  described, so a `password` field held a plausible 54-character string that an
  external reviewer disproved only by counting distinct values across sixteen
  records. Since 0.2.0 no field named `password` or `otp_secret` holds a string
  at all: a withheld value is `null` with a sibling `<field>_redacted: true`, and
  the genuinely-null case still gets no flag, which is the part of this bullet
  that survives unchanged. See ADR-003.
- **Annotations derived from the class**, recorded above as a control against a
  tool being mis-declared harmless one file at a time. It was, collectively:
  `destructiveHint` was mapped as "does this delete something", so all 32
  mutating non-`Destructive` tools claimed `false` where the protocol defines
  `false` as "only additive". `Update` and `Admin` carry `true` from 0.2.0. The
  deriving-from-the-class decision is what made that a one-function fix rather
  than 32 edits. See ADR-003.
...
five unpaginated collections (C2), the rack-contents claim later corrected (C4),
...
(B6). C4 was offered here as the cleanest case — a question the API cannot
answer gets no tool rather than an invented join — and the principle holds while
the example does not: the API does answer that question, on the rack record, and
declining to invent a join was never the same thing as telling callers to refuse
(see C4 and F8 in `spec-defects.md`). Article IV governs what may be _claimed_
about an interface; it does not license a claim that a documented endpoint
returns less than it does.
